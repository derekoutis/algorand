...
func WriteKeyToFile(k []byte, saveFileTo string) (err error) {
	return ioutil.WriteFile(saveFileTo, []byte(fmt.Sprintf("%x", k)), 0600)
}

func ReadKeyFromFile(loadFileFrom string) (k Key, err error) {
	k0, err := ioutil.ReadFile(loadFileFrom)
	if err != nil {
		return k, err
	}
	if len(k0) < 64 {
		return k, fmt.Errorf("File contents of %s insufficient", loadFileFrom)
	}
	k = common.FromHex(string(k0))
	return k, err
}
